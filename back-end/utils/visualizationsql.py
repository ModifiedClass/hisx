...
#三年收入对比
sqldisease='''SELECT y,
MAX(DECODE(m, '01', 收入,0)) AS Jan,
MAX(DECODE(m, '02', 收入,0)) AS Feb,
MAX(DECODE(m, '03', 收入,0)) AS Mar,
MAX(DECODE(m, '04', 收入,0)) AS Apr,
MAX(DECODE(m, '05', 收入,0)) AS May,
MAX(DECODE(m, '06', 收入,0)) AS Jun,
MAX(DECODE(m, '07', 收入,0)) AS Jul,
MAX(DECODE(m, '08', 收入,0)) AS Aug,
MAX(DECODE(m, '09', 收入,0)) AS Sep,
MAX(DECODE(m, '10', 收入,0)) AS Oct,
MAX(DECODE(m, '11', 收入,0)) AS Nov,
MAX(DECODE(m, '12', 收入,0)) AS Dec 
from
(select substr(Y.时间,0,4) as y,substr(Y.时间,-2,3) as m,门诊收入+住院收入 as 收入 from 
(select rownum as id,时间,门诊收入 from
((select to_char(A.登记时间,'yyyy-mm') as 时间,SUM(A.实收金额) AS 门诊收入
  from 门诊费用记录 A
 WHERE A.记录状态<>0
   AND A.记录性质 in (1,4)
   AND A.门诊标志=1
   and A.收费细目ID<>57961
   AND A.登记时间 between to_date(to_char((TRUNC(SYSDATE,'YYYY')-INTERVAL '2' YEAR),'yyyy-mm'),'yyyy-mm')
and to_date(to_char(add_months(trunc(sysdate,'yyyy'),12),'yyyy-mm'),'yyyy-mm')
   group by to_char(A.登记时间,'yyyy-mm')
   order by to_char(A.登记时间,'yyyy-mm')))) X
 ,  
(select rownum as id,时间,住院收入 from
((select to_char(B.收费时间,'yyyy-mm') as 时间,SUM(A.结帐金额) AS 住院收入
  from Y_病人住院结帐汇总 A,病人结帐记录 B
 WHERE  B.结帐类型=2
   AND A.结帐ID=B.ID
   AND B.收费时间 between to_date(to_char((TRUNC(SYSDATE,'YYYY')-INTERVAL '2' YEAR),'yyyy-mm'),'yyyy-mm')
and to_date(to_char(add_months(trunc(sysdate,'yyyy'),12),'yyyy-mm'),'yyyy-mm')
   group by to_char(B.收费时间,'yyyy-mm')
   order by to_char(B.收费时间,'yyyy-mm')))) Y
where X.id=Y.id) t
GROUP BY y
order by y'''
...
#当前在院病人数
sqlinhosnum='''
Select 普通门诊人次,急诊人次,专家门诊人次,在院人数,入院人次,出院人次,手术台次
From (Select Sum(Decode(B.号类, '普通', 1, 0)) As 普通门诊人次,Sum(Decode(B.号类, '急诊', 1, 0)) As 急诊人次,Sum(Decode(B.号类, '专家', 1, '知名专家', 1, 0)) As 专家门诊人次
          From 病人挂号记录 A, 挂号安排 B
          Where A.登记时间 Between to_date(to_char(sysdate, 'yyyy-mm-dd') || ' 00:00:00', 'yyyy-mm-dd hh24:mi:ss') and sysdate
          And A.执行部门id Is Not Null
          AND A.号别 = B.号码) a,
      (Select a.出院人次, b.入院人次
          From (Select Count(Distinct 病人id) As 出院人次
                    From 病案主页
                    Where 出院日期 Between to_date(to_char(sysdate, 'yyyy-mm-dd') || ' 00:00:00','yyyy-mm-dd hh24:mi:ss') and sysdate) a,
               (Select Count(Distinct 病人id) As 入院人次
                    From 病案主页
                    Where 入院日期 Between to_date(to_char(sysdate, 'yyyy-mm-dd') || ' 00:00:00', 'yyyy-mm-dd hh24:mi:ss') and sysdate) b
      ) b,       
      (Select Sum(Decode(收费类别, '5',实收金额,'6',实收金额,'7',实收金额,0)) As 药品收入,Sum(Decode(收费类别,'5',0,'6',0,'7',0,'Z', 0,实收金额)) As 医疗收入,
                Sum(Decode(收费类别, 'Z', 实收金额, 0)) As 其他收入,
                Sum(实收金额) As 合计收入
          From 病人费用记录
          Where 记录状态 <> 0
          and 登记时间 Between to_date(to_char(sysdate, 'yyyy-mm-dd') || ' 00:00:00', 'yyyy-mm-dd hh24:mi:ss') and sysdate) c,
       
      (Select count(*) 在院人数
           From 病案主页
           Where 出院日期 is null
           and 主页ID > 0
           and 出院病床 is not null) d, 
      (SELECT COUNT(*) 手术台次
           FROM (Select *
                      From 病人手麻主页 x,病人手麻状态 y
                      Where x.开始时间 BETWEEN to_date(to_char(sysdate, 'yyyy-mm-dd') ||  ' 00:00:00',  'yyyy-mm-dd hh24:mi:ss') and sysdate
                      And x.执行科室id + 0 = '211'
                      And y.手麻主页id = x.ID
                      And y.结束操作 = 1
                      And x.手术状态 = 7)
...
#当月门诊金额统计
sqloutpatientbythismon='''select sum(费用) as 费用,名称 from
(select sum(A.实收金额) as 费用,
decode(B.名称,
'中成药','药品费',
'西药费','药品费',
'中草药','药品费',
...
and 记录状态=1
and 门诊标志=1
AND 登记时间 BETWEEN trunc(sysdate,'mm')
AND trunc(sysdate+1)
group by B.名称)group by 名称'''
...
#当月每天门诊人次
sqlnumbydaym='''select 普通门诊人次+专家门诊人次 as 门诊,时间  from
(Select to_char(登记时间,'dd') as 时间,Sum(Decode(B.号类, '普通', 1, 0)) As 普通门诊人次,Sum(Decode(B.号类, '专家', 1, '知名专家', 1, 0)) As 专家门诊人次
          From 病人挂号记录 A, 挂号安排 B
          Where A.登记时间 BETWEEN trunc(sysdate,'mm') AND trunc(sysdate+1)
          And A.执行部门id Is Not Null
          AND A.号别 = B.号码
          group by to_char(登记时间,'dd')
          order by to_char(登记时间,'dd'))'''

#当月每天急诊人次
sqlnumbydaye='''Select Sum(Decode(B.号类, '急诊', 1, 0)) As 急诊人次,to_char(登记时间,'dd') as 时间
          From 病人挂号记录 A, 挂号安排 B
          Where A.登记时间 BETWEEN trunc(sysdate,'mm') AND trunc(sysdate+1)
          And A.执行部门id Is Not Null
          AND A.号别 = B.号码
...
#近三年收入对比
sqlIncomeOfComparison='''
select Y.时间,门诊收入+住院收入 as 收入 from 
(select rownum as id,时间,门诊收入 from
((select to_char(A.登记时间,'yyyy-mm') as 时间,SUM(A.实收金额) AS 门诊收入
  from 门诊费用记录 A
 WHERE A.记录状态<>0
